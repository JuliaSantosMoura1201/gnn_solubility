...
def main(args):
	...
	for epoch in range(args.num_epoches):
		...
		train_metrics = evaluate_classification_multi(
			y_list=y_list,
			pred_list=pred_list
		)
		...
		with torch.no_grad():
			...
			for i, batch in enumerate(valid_loader):
				...
				if epoch < args.swa_start:
					...
					pred, alpha = swa_model(graph, training=False)
					pred_list.append(pred)
			...
			valid_metrics = evaluate_classification_multi(
				y_list=y_list,
				pred_list=pred_list
			)
			...
			test_metrics = evaluate_classification_multi(
				y_list=y_list,
				pred_list=pred_list
			)
...
if __name__ == '__main__':
	...
	parser.add_argument('--job_title', type=str, default='GNN_classification', 
	...
	parser.add_argument('--swa_start', type=int, default=100, 
						help='Step of starting SWA')
